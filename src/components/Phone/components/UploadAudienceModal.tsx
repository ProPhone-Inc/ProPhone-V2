...
import { X, Upload, FileSpreadsheet, File, AlertTriangle, Check } from 'lucide-react';
...
interface UploadAudienceModalProps {
  onClose: () => void;
  onUpload: (file: File, name: string, count: number) => void;
}

export function UploadAudienceModal({ onClose, onUpload }: UploadAudienceModalProps) {
  ...
  const validateFile = (file: File): boolean => {
    const validTypes = [
      ...
      'text/csv',
      'application/csv',
      'text/plain', // Some systems save CSV as text/plain
      '' // Some systems don't set type for CSV
    ];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedFile || !audienceName.trim()) {
      setError('Please provide an audience name and select a file');
      return;
    }
    ...
    try {
      ...
      setShowMapping(true); // Move directly to mapping without showing second modal
    } catch (error) {
  ...
  const handleConfirmMapping = async (mappings: Record<string, string>) => {
    ...
    const taskId = addTask({
      ...
      name: `Processing ${audienceName}`,
      status: 'queued',
      progress: 0,
      stage: 'Preparing data',
      message: 'Starting audience processing...'
    });

    // Add initial notification
    addNotification({
      title: 'Processing Audience',
      message: `Starting to process ${audienceName}`,
      type: 'announcement',
      priority: 'medium'
    });
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i += batchSize) {
        ...
        for (const row of batch) {
          ...
          let rowPhones: string[] = [];

          // Check all phone fields
          for (let phoneNum = 1; phoneNum <= 5; phoneNum++) {
            const phoneField = `phone${phoneNum}`;
            const phoneColumn = mappings[phoneField];
          ...
          if (!isDuplicate && rowPhones.length > 0) {
            rowPhones.forEach((phone, idx) => {
              uniquePhones.add(phone);
              contact[`phone${idx + 1}`] = phone;
            });

            Object.entries(mappings).forEach(([field, column]) => {
              if (!field.startsWith('phone') && row[column]) {
                contact[field] = row[column].toString().trim();
              }
            });
        ...
        processedContacts.push(...batchContacts);
        ...
        await updateTask(taskId, {
          ...
          message: `Processed ${Math.min(i + batchSize, rows.length)} of ${rows.length} contacts (${duplicateCount} duplicates, ${blankCount} invalid)`
        });
      ...
      await updateTask(taskId, {
        ...
        progress: 100,
        stage: 'Complete',
        message: `Processed ${processedContacts.length} contacts (${duplicateCount} duplicates, ${blankCount} invalid)`,
        endTime: Date.now()
      });

      // Send completion notification
      await addNotification({
        title: 'Audience Processing Complete',
        message: `${audienceName} has been processed with ${processedContacts.length} contacts`,
        type: 'announcement',
        priority: 'low'
      });

      // Pass processed data to parent
      await onUpload(selectedFile!, audienceName, processedContacts.length);
      ...
      await addNotification({
        title: 'Audience Processing Failed',
      ...
      <div className="relative bg-zinc-900 border border-[#B38B3F]/30 rounded-xl shadow-2xl w-full max-w-xl transform animate-fade-in">
        <div className="flex items-center justify-between p-6 border-b border-[#B38B3F]/20">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-white">Upload Audience</h2>
              <p className="text-white/60">Import contacts from Excel or CSV file</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 border-b border-[#B38B3F]/20">
          <label className="block text-white/70 text-sm font-medium mb-2">
            Audience Name <span className="text-[#FFD700]">*</span>
          </label>
          <input
            type="text"
            value={audienceName}
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="flex justify-end space-x-3 mt-6">
            ...
            <button
              type="submit"
              disabled={!selectedFile || !audienceName.trim() || isUploading}
